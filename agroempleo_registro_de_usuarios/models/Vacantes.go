...
type Vacantes struct {
	Id                       int           `orm:"column(Id_empleo);pk"`
...
func init() {
	orm.RegisterModel(new(Vacantes))
}

// AddVacantes insert a new Vacantes into database and returns
// last inserted Id on success.
func AddVacantes(m *Vacantes) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetVacantesById retrieves Vacantes by Id. Returns error if
// Id doesn't exist
func GetVacantesById(id int) (v *Vacantes, err error) {
	o := orm.NewOrm()
	v = &Vacantes{Id: id}
...
// GetAllVacantes retrieves all Vacantes matches certain condition. Returns empty list if
// no records exist
...
// UpdateVacantes updates Vacantes by Id and returns error if
// the record to be updated doesn't exist
func UpdateVacantesById(m *Vacantes) (err error) {
	o := orm.NewOrm()
	v := Vacantes{Id: m.Id}
...
// DeleteVacantes deletes Vacantes by Id and returns error if
// the record to be deleted doesn't exist
func DeleteVacantes(id int) (err error) {
	o := orm.NewOrm()
	v := Vacantes{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Vacantes{Id: id}); err == nil {
